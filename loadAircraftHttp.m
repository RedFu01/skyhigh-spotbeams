...
%   Detailed explanation goes here
    ...
    %if(scenario ~= 'rural' && scenario ~= 'urban')
    ...
    try
        ...
        if(length(varargin) > 0)
            ...
            url = strcat('http://localhost:', port, '/spotbeam?timestamp=',num2str(timestamp), '&db=', db);
            url = strjoin(url, '');
        else
            url = strcat('http://orion6:3000/spotbeam?timestamp=',num2str(timestamp), '&db=', db);
        end
